...
################## matlab noddi and hormones ##################
file_list <- c("matlab_icvf_all.csv",
               "fa_all.csv", "md_all.csv", "ad_all.csv", "rd_all.csv")
hormone_list <- c("progesterone", "estradiol", "testosterone", "cortisol")
...
ROI_interest <- c("AF", "ATR", "CG", "CST", "FPT", "ICP", "IFO", "ILF", "OR", "POPT", "SCP", "SLF_I",
                  ...
                  "CC_body", "CC_IS")
df_final_hormone <- data.frame(ROI=ROI_interest, stringsAsFactors=FALSE)
...
for (metric in file_list) {
  ...
  importDat_sub <- importDat_sub[!((importDat_sub$fsid=="10003_scan3") 
                                   ...
                                   | (importDat_sub$fsid=="10001_scan1")),]
  ...
  for (horm in hormone_list) {
    ...
    for (i in 1:length(list_subregions_long)) {
      ...
      horm_z_tmp <- paste0(horm, "_z")
      fit_func_z <- formula(paste0("y_z_tmp ~ hemi + age_z + motion_rel_z + ", horm_z_tmp))
      fit_z_tmp <- lme(fit_func_z, random = ~ 1 | sub, data=data, na.action=na.omit)
      sum_tmp_z <- summary(fit_z_tmp)
      tmp_p <- sum_tmp_z$tTable[horm_z_tmp, "p-value"]
      beta_lower_tmp <- intervals(fit_z_tmp, which=c("fixed"))['fixed']$fixed[horm_z_tmp, "lower"]
      beta_tmp <- intervals(fit_z_tmp, which=c("fixed"))['fixed']$fixed[horm_z_tmp, "est."]
      beta_upper_tmp <- intervals(fit_z_tmp, which=c("fixed"))['fixed']$fixed[horm_z_tmp, "upper"]
      print(intervals(fit_z_tmp, which=c("fixed")))
      # r^2
      print("r^2")
      # print(r2beta(fit_tmp, method ='nsj'))
      idx_tmp_r2 <- r2beta(fit_tmp, method ='nsj')[,"Effect"] == horm
      r2_tmp <- r2beta(fit_tmp, method ='nsj')[idx_tmp_r2,"Rsq"]
      r2_lower_tmp <- r2beta(fit_tmp, method ='nsj')[idx_tmp_r2,"lower.CL"]
      r2_upper_tmp <- r2beta(fit_tmp, method ='nsj')[idx_tmp_r2,"upper.CL"]
      # append
      df_result_ges[nrow(df_result_ges) + 1,] = c(data_dv, unname(tmp_p), b_tmp, 
                                                  ...
                                                  unname(r2_tmp), unname(r2_lower_tmp), unname(r2_upper_tmp))
    }
    ...
    for (i in tail(names(dat_total_subset),3)) {
      fit_func <- formula(paste(i, "~ age + motion_rel +", horm))
      ...
      horm_z_tmp <- paste0(horm, "_z")
      fit_func_z <- formula(paste0("y_z_tmp ~ age_z + motion_rel_z + ", horm_z_tmp))
      fit_z_tmp <- lme(fit_func_z, random = ~ 1 | sub, data=dat_total_subset, na.action=na.omit)
      sum_tmp_z <- summary(fit_z_tmp)
      tmp_p <- sum_tmp_z$tTable[horm_z_tmp, "p-value"]
      beta_lower_tmp <- intervals(fit_z_tmp, which=c("fixed"))['fixed']$fixed[horm_z_tmp, "lower"]
      beta_tmp <- intervals(fit_z_tmp, which=c("fixed"))['fixed']$fixed[horm_z_tmp, "est."]
    ...
    new_column_name_beta <- paste0(current_metric, "_", horm, "_beta")
...
for (metric in file_list) {
  ...
  importDat_sub <- importDat_sub[!((importDat_sub$fsid=="10003_scan3") 
                                   ...
                                   | (importDat_sub$fsid=="10011_scan2")),]
  dat_total_subset <- importDat_sub
  dat_total_subset$distance_hair_sample <- dat_total_subset$segment_sample_use - 1
  ...
  dat_total_subset$age_z <- scale(dat_total_subset$age, center = TRUE, scale = TRUE)
  dat_total_subset$motion_rel_z <- scale(dat_total_subset$motion_rel, center = TRUE, scale = TRUE)
  dat_total_subset$segment_sample_use_z <- scale(dat_total_subset$segment_sample_use, center = TRUE, scale = TRUE)
  dat_total_subset$distance_hair_sample_z <- scale(dat_total_subset$distance_hair_sample, center = T, scale = T)
  dat_total_subset$cortisol_win_centered_z <- scale(dat_total_subset$cortisol_win_centered, center = TRUE, scale = TRUE)
  dat_total_subset$cortisone_win_centered_z <- scale(dat_total_subset$cortisone_win_centered, center = TRUE, scale = TRUE)
  dat_total_subset$progesterone_win_centered_z <- scale(dat_total_subset$progesterone_win_centered, center = TRUE, scale = TRUE)
  list_subregions <- list()
  list_subregions_long <- list()
  for (trac in names(dat_total_subset)) {
    if (grepl('_left', trac)) {
      ...
      trac_right <- sub("_left", "_right", trac)
      idx_right <- which(trac_right == names(dat_total_subset))
      sub_col_name <- c(trac, trac_right, "cortisol_win_centered", "cortisone_win_centered", "progesterone_win_centered",
                        "distance_hair_sample",
                        "ges_time", "age", "sub", "motion_rel", "cortisol_win_centered_z", "cortisone_win_centered_z",
                        "progesterone_win_centered_z", "distance_hair_sample_z", "ges_time_z", "scan", "age_z", "motion_rel_z")
  ...
  for (horm in hormone_list) {
    ...
    for (i in 1:length(list_subregions_long)) {
      ...
      data_dv <- names(data)[length(data)]
      print(data_dv)
      fit_func <- formula(paste(data_dv, "~ hemi + distance_hair_sample + motion_rel + age + ", horm))
      ...
      fit_func_z <- formula(paste0("y_z_tmp ~ hemi + distance_hair_sample_z + motion_rel_z + age_z + ", horm_z_tmp))
      fit_z_tmp <- lme(fit_func_z, 
                       random = ~ 1 | sub, 
                       data=data, 
                       na.action=na.omit)
      sum_tmp_z <- summary(fit_z_tmp)
    ...
    for (i in tail(names(dat_total_subset),3)) {
      fit_func <- formula(paste(i, "~ distance_hair_sample + motion_rel + age + ", horm))
      fit_tmp <- lme(fit_func, random = ~ 1 | sub,
                     data=dat_total_subset, na.action=na.omit)
      print(summary(fit_tmp))
      sum_tmp <- summary(fit_tmp)
      ...
      y_z_tmp <- scale(dat_total_subset[,idx_tmp], center = TRUE, scale = TRUE)
      horm_z_tmp <- paste0(horm, "_z")
      fit_func_z <- formula(paste0("y_z_tmp ~ distance_hair_sample_z + motion_rel_z + age_z + ", horm_z_tmp))
      fit_z_tmp <- lme(fit_func_z, random = ~ 1 | sub, 
